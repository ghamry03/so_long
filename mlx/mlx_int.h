...
typedef struct	mlx_move_list_s
{
  int			width;
  int			height;
  char			*buffer;
  GLfloat		vertexes[8];
  struct mlx_move_list_s	*next;
} mlx_move_list_t;


typedef struct	mlx_move_ctx_s
{
  GLuint		texture;
  GLuint		vbuffer;
  mlx_move_list_t	*move;
  struct mlx_move_ctx_s	*next;
} mlx_move_ctx_t;

typedef struct	mlx_win_list_s
{
  void			*winid;
  mlx_move_ctx_t		*move_list;
  int			nb_flush;
  int			pixmgt;
...
typedef struct		mlx_ptr_s
{
  ...
  mlx_move_list_t	*move_list;
  void			(*loop_hook)(void *);
  void			*loop_hook_data;
  void			*loop_timer;
  mlx_move_list_t	*font;
...
int	mlx_destroy_image(mlx_ptr_t *mlx_ptr, mlx_move_list_t *move_ptr);
void	*mlx_new_image();
