...
- (void) mlx_gl_draw_move:(mlx_move_list_t *)move andCtx:(mlx_move_ctx_t *)movectx andX:(int)x andY:(int)y
{
  ...
  glUniform2f(glsl.loc_image_size, move->width, -move->height);
...
- (void) mlx_gl_draw_font:(mlx_move_list_t *)move andCtx:(mlx_move_ctx_t *)movectx andX:(int)x andY:(int)y andColor:(int)color glyphX:(int)gx glyphY:(int)gy
{
  GLfloat color_tab[4];
  ...
  glUniform2f(glsl.loc_font_atlassize, move->width, move->height);
  ...
  if (win_to_del->pixmgt)
    {
      ...
      ctx = win_to_del->move_list;   // should be null anyway if no pixel management
      while (ctx)
	{
