...
// only work for mlx mac or move 32bpp
int	mipng_fill_move(mlx_move_list_t *move, unsigned char *buf, png_info_t *pi)
{
  ...
  ibuf = (unsigned char *)move->buffer;
  iline = move->width * UNIQ_BPP;
  ilen = move->width * move->height * UNIQ_BPP;
  blen = move->width * move->height * pi->bpp + move->height;
  ipos = 0;
  bpos = 0;
...
int	mipng_data(mlx_move_list_t *move, unsigned char *dat, png_info_t *pi)
{
  unsigned int	len;
  int		b_pos;
  ...
  b_pos = 0;
  if (!(buffer = malloc((long long)move->width*(long long)move->height*(long long)pi->bpp + move->height)))
    err(1, "Can't malloc");
  ...
  while (mipng_is_type(dat, "IDAT"))
      ...
      while (z_strm.avail_out == 0)
	{
	  ...
	  if (b_pos + Z_CHUNK - z_strm.avail_out > move->width*move->height*pi->bpp+move->height)
	    {
	      inflateEnd(&z_strm);
	      return (ERR_DATA_MISMATCH);
	    }
  ...
  inflateEnd(&z_strm);
  if (b_pos != move->width*move->height*pi->bpp+move->height)
    {
      //      printf("pb : bpos %d vs expected %d\n", b_pos, move->width*move->height*pi->bpp+move->height);
      return (ERR_DATA_MISMATCH);
    }
  if ((ret = mipng_fill_move(move, buffer, pi)))
    return (ret);
  return (0);
}
...
mlx_move_list_t	*mlx_int_parse_png(mlx_ptr_t *xvar, unsigned char *fptr, int size)
{
  int		err;
  unsigned char *hdr;
  unsigned char *dat;
  ...
  if (!(move = mlx_new_image(xvar, pi.width, pi.height)))
    {
      warnx("mlx PNG error : Can't create mlx image");
      return ((mlx_move_list_t *)0);
    }
  if ((err = mipng_data(move, dat, &pi)))
    {
      mlx_destroy_image(xvar, move);
      warnx("mlx PNG error : %s", mipng_err[err]);
      return ((mlx_move_list_t *)0);
    }
  return (move);
...
void	*mlx_png_file_to_image(mlx_ptr_t *xvar, char *file, int *width, int *height)
{
  ...
  if ((move = mlx_int_parse_png(xvar, ptr, size)))
    {
      *width = move->width;
      *height = move->height;
    }
