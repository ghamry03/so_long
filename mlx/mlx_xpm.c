...
#define	RETURN	{ if (colors) free(colors); if (tab) free(tab); \
		  if (colors_direct) free(colors_direct); \
                  if (move) mlx_destroy_image(xvar, move);   \
                  return ((void *)0); }
...
void	mlx_int_xpm_set_pixel(mlx_move_list_t *move, char *data, int opp, int col, int x)
{
  /*
  int	dec;
...
void	*mlx_int_parse_xpm(mlx_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  ...
  while (i--)
      ...
      if ((rgb_col = mlx_int_get_text_rgb(tab[j], tab[j+1]))==-1)
	{
	  if (!(clip_data = malloc(4*width*height)) ||   // ok, nice size ..
	      !(clip_move = XCreateImage(xvar->display, xvar->visual,
  ...
  if (!(move = mlx_new_image(xvar,width,height)))
    RETURN;
  //opp = move->bpp/8;
  opp = 4;
  ...
  while (i--)
      ...
      while (x<width)
	{
	  ...
	  //	  if (col==-1)
	  //	    XPutPixel(clip_move, x, height-1-i, 0);
	  //	  else
	  if (col==-1)
	    col = 0xFF000000;
	  mlx_int_xpm_set_pixel(move, data, opp, col, x);
	  x ++;
	}
      //      data += move->size_line;
      data += move->width*4;
  ...
  if (clip_data)
    {
      if (!(clip_pix = XCreatePixmap(xvar->display, xvar->root,
					   width, height, 1)) )
	RETURN;
      move->gc = XCreateGC(xvar->display, clip_pix, 0, &xgcv);
      XPutImage(xvar->display, clip_pix, move->gc, clip_move,
		0, 0, 0, 0, width, height);
      XFreeGC(xvar->display, move->gc);
      xgcv.clip_mask = clip_pix;
      xgcv.function = GXcopy;
      xgcv.plane_mask = AllPlanes;
      move->gc = XCreateGC(xvar->display, xvar->root, GCClipMask|GCFunction|
			  GCPlaneMask, &xgcv);
      XSync(xvar->display, False);
      XDestroyImage(clip_move);
    }
  */
  if (colors)
    free(colors);
...
void	*mlx_xpm_file_to_image(mlx_ptr_t *xvar,char *file,int *width,int *height)
{
  ...
  if ((move = mlx_int_parse_xpm(xvar,ptr,size,mlx_int_get_line)))
    {
      *width = move->width;
      *height = move->height;
    }
...
void	*mlx_xpm_to_image(mlx_ptr_t *xvar,char **xpm_data,int *width,int *height)
{
  ...
  if ((move = mlx_int_parse_xpm(xvar,xpm_data,0,mlx_int_static_line)))
    {
      *width = move->width;
      *height = move->height;
    }
